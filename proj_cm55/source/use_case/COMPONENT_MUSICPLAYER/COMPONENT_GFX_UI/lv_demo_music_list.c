...
void shuffle_list(void)
{
    ...
    for(int i = 0; i < ACTIVE_TRACK_CNT; i++)
    {
        ...
        uint32_t t = lv_demo_music_get_track_length(playback_order[i]);
        const char * title = lv_demo_music_get_title(playback_order[i]);
        const char * artist = lv_demo_music_get_artist(playback_order[i]);
        lv_snprintf(time, sizeof(time), "%"LV_PRIu32":%02"LV_PRIu32" ", t / 60, t % 60);

        lv_obj_t * title_label = lv_obj_get_child(btn, 1);
...
static void btn_click_event_cb(lv_event_t * e)
{
    ...
    if(lv_obj_has_state(btn, LV_STATE_CHECKED))
    {
        ...
        lv_demo_music_pause();
        lv_demo_music_list_button_check(idx, false);
    }
    else
    {
        track_idx = (uint8_t)playback_order[idx];
        music_player_q_data.data = &track_idx;
        music_player_q_data.data_len = sizeof(track_idx);
        music_player_q_data.cmd = I2S_PLAYBACK_PLAY_TRACK_INDEX;
        lv_demo_music_play(idx);
        lv_demo_music_list_button_check(idx, true);
    }
